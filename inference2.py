...
class AffinityPredictor:
    ...
    def img_to_aff(self, img, zarr_path):
        ...
        z = zarr.open_group("tmp_" + zarr_path, mode='w')
        ...
        if self.compute_backend == "local":
            for chunk in tqdm(chunked_patch_coordinates):
                self.predict_aff_patches_chunked(chunk, img, "tmp_" + zarr_path)
            ...
            tasks = [dask.delayed(self.predict_aff_patches_chunked)(chunk, img, "tmp_" + zarr_path) for chunk in chunked_patch_coordinates]
            futures = persist(tasks)
            progress(futures)  # progress bar
            compute(futures)

        tmp_sum_pred = da.from_zarr(f"tmp_{zarr_path}/sum_pred")
        tmp_sum_weight = da.from_zarr(f"tmp_{zarr_path}/sum_weight")
        aff = tmp_sum_pred / tmp_sum_weight
        aff.to_zarr(zarr_path, overwrite=True)

        shutil.rmtree("tmp_" + zarr_path)
...
class Postprocessing:
    ...
    def merge_and_relabel(self, fragment_agglomeration, zarr_patched, zarr_final, chunks, aff_shape):
        zarr_chunk_size = min(self.chunk_cube_size, 512)
